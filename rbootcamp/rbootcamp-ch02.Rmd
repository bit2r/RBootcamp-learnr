...
```{r exc-02-01-new, exercise=TRUE, exercise.blanks = "___+"}
library(dplyr)
...
```{r exc-02-02, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r exc-02-03, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r exc-02-quiz, eval = FALSE}
ggplot(pets, aes(x=animal, fill=shotsCurrent)) + 
  geom_bar(color="black")
```

```{r pet-ggplot, echo = FALSE, out.width = "100%", out.height="70%"}
knitr::include_graphics("images/pet_black.png")
```


```{r ggplot-aes-more-mapping, echo = FALSE}
question("위 예제처럼 `geom_bar()` 함수에서 `color`에 `black`을 매핑하면 어떻게 동작하나요?",
         answer("막대의 내부 색상의 기본값을 검정색으로 지정합니다.", 
                message = "그렇지 않습니다. 그래프에서 어떤 것이 검게 표현되었나요?"),
         answer("그래프의 텍스트를 검은색으로 지정합니다.", 
                message = "그렇지 않습니다. 무엇이 검은색으로 매핑되는지 그래프를 확인하세요."),
         answer("막대의 윤곽선을 검은색으로 그립니다.",
                message = "오! 맞습니다, 우리는 검은색으로 막대의 윤곽을 그립니다.", 
                correct = TRUE),
         allow_retry = TRUE,
...
```{r exc-02-06, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")

ggplot(pets, aes(x=animal,fill=shotsCurrent)) + 
  geom_bar(position= ____, color="black")
...
```{r exc-02-07, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r exc-02-08, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r ggplot-facet-review, eval = FALSE}
ggplot(pets, aes(x=name)) + geom_bar() + 
  facet_wrap(facets=~shotsCurrent, scale="free_x") +
  theme(axis.text.x = element_text(angle=45))
```
...
```{r ggplot-facet-quiz, echo = FALSE}
question("위 코드를 보고 패싯을 통해서 우리는 다음을 수행할 수 있습니다.:",
         answer("플롯의 포인트를 예쁘게 출력", 
                message = "패싯은 그런 종류의 기하적 표현이 아닙니다!"),
         answer("또 다른 범주형 변수로 그래프의 계층화", 
                message = "패싯은 추가 변수를 계층화하여 정보를 시각화하는 방법입니다.", 
                correct = TRUE),
         answer("x-축에 또다른 범주 추가",
                message = "그렇지 않습니다."),
         allow_retry = TRUE,
         random_answer_order = TRUE
)
...
```{r exc-02-09, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r pet-ggplot-shot, echo = FALSE, out.width = "100%", }
knitr::include_graphics("images/pet_shot.png")
```
...
```{r ggplot-boxplot-quiz, echo = FALSE}
question("이 플롯은 우리에게 무엇을 말해주고 있나요? 예방 접종한 동물과 그렇지 않은 동물의 체중 차이가 있나요?",
         answer("평균에 약간의 차이가 있지만 그 차이는 유의해(significant) 보이지 않습니다.", 
                message = "예, 이 경우는 사분위수 범위(IQR, InterQuartile Range)가 겹쳐서 유의하지 않을 것입니다. 사분위수 범위는 3사분위수(75%분위)와 1사분위(25%분위) 사이의 거리를 의미하며, 데이터의 퍼짐 정도를 파악하는 통계량입니다.", 
                correct = TRUE),
         answer("평균에는 차이가 없습니다.", 
                message = "판정 나름이지만, 그룹간의 평균에는 약간의 차이가 있습니다."),
         answer("평균에 큰 차이가 있으며, 그 차이는 아마도 통계적으로 유의할 것입니다.",
                message = "사실이 아닙니다. 사분위수 범위가 겹치므로 평균은 큰 차이가 없을 것입니다."),
         allow_retry = TRUE,
         random_answer_order = TRUE
)
```
...
```{r exc-02-11, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r exc-02-12, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
...
```{r exc-02-13, exercise=TRUE, exercise.blanks = "___+"}
library(ggplot2)
pets <- read.csv("data/pets.csv")
