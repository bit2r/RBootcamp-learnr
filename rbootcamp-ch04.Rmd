...
```{r dataframe-ch04, echo = FALSE, out.width = "100%", }
...
```{r exc-04-02, exercise=TRUE, exercise.blanks = "___+"}
...
```{r exc-04-02-quiz}
question("콘솔에서 `head()` 또는 `summary()`를 사용하여 `dem_score` 데이터 세트를 살펴봅니다. 이 데이터 세트는 깔끔하다고 생각하십니까?",
         answer("예, 각 행들을 분리된 개별 관측치로 간주합니다.", 
                message = "컬럼을 살펴보세요. - 행이 개별 관측치로 간주됩니까, 아니면 시점별 여러 관측치로 간주됩니까?"),
         answer("아니요, 각 컬럼은 개별 관측치가 아니라 실제로는 하나의 관측치에 대응하는 여러 컬럼들이 포함되어 있습니다.", 
                message = "예, 각 행과 연도별 컬럼들 묶음을 개별의 관측치로 간주해야 합니다.", 
                correct = TRUE),
         allow_retry = TRUE
)
```
...
```{r ch04-pivot-longer, eval = FALSE, echo = TRUE}
...
```{r exc-04-03, exercise=TRUE, exercise.blanks = "___+"}
library(dplyr)
library(tidyr)
library(stringr)
dem_score <- read.csv("data/dem_score.csv")
...
```{r ch04-pivot-wider, eval = FALSE, echo=TRUE}
...
```{r exc-04-04, exercise=TRUE, exercise.blanks = "___+"}
dem_score <- read.csv("data/dem_score.csv")
library(dplyr)
library(tidyr)
library(stringr)
...
```{r ch04-separate-tibble, eval = FALSE, echo = TRUE}
# A tibble: 5 x 3
...
```{r exc-04-05, exercise=TRUE, exercise.blanks = "___+"}
library(tidyr)
library(dplyr)
library(readr)
health_code_example <- 
...
```{r ch04-wider-dataset, eval = FALSE, echo=TRUE}
...
```{r exc-04-06, exercise=TRUE, exercise.blanks = "___+"}
library(tidyr)
library(dplyr)
library(ggplot2)
...
```{r exc-04-06-hint-1}
먼저 `filter()` 문을 사용하고 나서 `group_by/summarize` 문을 사용한다.
```
...
```{r ch04-07-all-in-one, eval = FALSE, echo=TRUE}
...
```{r exc-04-07, exercise=TRUE, exercise.blanks = "___+"}
library(tidyr)
library(dplyr)
library(ggplot2)
...
```{r mousetime-ch04, echo = FALSE, out.width = "100%", }
knitr::include_graphics("images/mouse_time.png")
```


```{r exc-04-08-quiz}
question("어떤 처리 전과 후에 빔이 머무는 평균 시간에 차이가 있나요?",
         answer("아니요, 시간의 차이는 거의 없습니다.", 
                message = "돌아가서 그래프를 다시 살펴보세요."),
         answer("예, 구간은 겹쳤지만, 평균은 확연히 다릅니다.", 
                message = "사분위수 범위는 겹쳤지만 중위수는 차이가 있습니다.", 
                correct = TRUE),
         allow_retry = TRUE
)
```
