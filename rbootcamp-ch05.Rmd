...
```{r exc-05-01, exercise=TRUE, exercise.blanks = "___+"}
...
```{r exc-05-02, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
library(ggplot2)
...
```{r exc-05-03, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
...
```{r exc-05-04-quiz}
question("어부와 어부가 아닌 두 그룹 사이에 차이가 있습니까?",
         answer("아니요, 차이가 없습니다. 평균이 너무 비슷합니다.", 
                message = "다시 한번 살펴보세요."),
         answer("예, 차이가 있습니다. 범위는 겹치지만 평균은 분명히 차이가 있습니다.", 
                message = "어부의 총 수은 값이 더 높습니다.", 
                correct = TRUE),
         allow_retry = TRUE
)
```
...
```{r exc-05-06, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
library(broom)
...
```{r exc-05-07, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
library(ggplot2)
...
```{r exc-05-08, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
...
```{r exc-05-09-quiz}
question("다중 선형회귀 모델의 결과에서 주당 생선 섭취 횟수의 회귀계수(`Estimate` 컬럼)은 무엇을 의미하나요?",
         answer("생선을 먹을 때마다 수은을 섭취할 가능성이 1.02배 더 높아집니다.", 
                message = "아니오. 효과는 가중되지 않습니다."),
         answer("매주 추가되는 생선 섭취 횟수는 동일한 체중의 어부나 어부가 아닌 사람들의 수은 수준을 0.106mg/g 증가시킬 것으로 예상됩니다.", 
                message = "체중과 어부 여부를 조정하여, 주당 생선 섭취 횟수가 가산 효과로 모델링됩니다.", 
                correct = TRUE),
         answer("일주일에 한 번 더 어분을 먹는 어부는 0.106mg/g의 더 높은 수은 수치를 가질 것으로 예상됩니다.", 
                message = "그렇지 않습니다. 이 예상 증가율은 어부뿐만 아니라 비 어부에게도 적용됩니다."),
         allow_retry = TRUE
)
```
...
```{r exc-05-10, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
library(broom)
...
```{r exc-05-11, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
library(broom)
...
```{r exc-05-12-quiz}
question("어부가 더 높은 수준의 수은과 관련있다고 얼마나 확신합니까?",
         answer("확실합니다, 나는 어부가 되고 싶지 않아요!", 
                message = "정말이세요?"),
         answer("자신 없습니다, 여기에는 다소 혼란스러운 요소가 있습니다. 아마도 생선을 덜 먹어야 할까요?", 
                message = "총 수은의 수준은 생선의 섭취량과 관련있어 보이며, 다중 선형회귀 모델에서 이를 조정한 후의 어부 여부는 더 이상 총 수은의 수준과 관련되지 않습니다.", 
                correct = TRUE),
         allow_retry = TRUE
)
```
...
```{r exc-05-13, exercise=TRUE, exercise.blanks = "___+"}
suppressPackageStartupMessages(library(dplyr))
library(broom)
library(ggplot2)
